...
test('header moves account controls into the sidebar footer and exposes editable new-thread selectors', async ({ page }) => {
  ...
  const controlBox = await page.locator('.header-control-row').boundingBox()
  const mainBox = await page.locator('.content-main').boundingBox()
  expect(controlBox).not.toBeNull()
  expect(mainBox).not.toBeNull()
  expect((controlBox?.x ?? 0)).toBeLessThanOrEqual((mainBox?.x ?? 0) + 32)
...
test('thread header shows read-only server/project selectors and lower-right panel toggles', async ({ page }) => {
  ...
  const threadControlBox = await page.locator('.header-control-row').boundingBox()
  const titleBox = await page.locator('.header-thread-title').boundingBox()
  const panelActionsBox = await page.locator('.header-panel-actions').boundingBox()
  expect(threadControlBox).not.toBeNull()
  expect(titleBox).not.toBeNull()
  expect(panelActionsBox).not.toBeNull()
  expect((threadControlBox?.x ?? 0)).toBeLessThan((panelActionsBox?.x ?? 0))
  expect((titleBox?.x ?? 0)).toBeLessThan((panelActionsBox?.x ?? 0))
