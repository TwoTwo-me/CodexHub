...
test('settings page manages connector bootstrap lifecycle end-to-end', async ({ page }) => {
  ...
  await expect(page.getByText(/shell history/i)).toBeVisible()
  ...
  await page.locator('.settings-install-card .settings-secondary-button').first().click()
  await expect(page.locator('.settings-install-card .settings-code-block').first()).toHaveValue(/reissued-/)
  await expect(page.getByLabel('Suggested install command')).toHaveValue(/--token "reissued-/)
